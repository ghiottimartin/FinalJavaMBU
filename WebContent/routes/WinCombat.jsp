...
<body>
 ...
 %></li>
	...
	<h1 align="center">
		...
		<%=p.getNombre()%></h1>
	<div class="labels">
		<label>Tu nivel es: <%=p.getId_nivel()%></label> <label>Tu
			Vida es: <%=p.getVida()%></label> <label>Tu Energia es: <%=p.getEnergia()%></label>
		<label>Tu Defensa es: <%=p.getDefensa()%></label> <label>Tu
			Evasion es: <%=p.getEvasion()%></label>
	</div>
	<form class="formBotones" method="post" action="Win">
		<button name="next_combate" class="btn btn-danger btn-lg"
			type="submit">Siguiente combate</button>
		<button name="guardar" class="btn btn-success btn-lg" type="submit">Guardar
			partida</button>
		<button name="editarAtaques" class="btn btn-success btn-lg"
			type="submit">Editar Ataques</button>
	</form>
